...
clear
addpath('.\')
rootaddr = 'G:\小王子数据\derivation\FR\Runs';
cd(rootaddr)
storyList = dir('*run*');
% Maskfile = 'E:\Little Prince\mask\Reslice444__Reslice_Reslice_GreyMask_02_61x73x61.img';
Maskfile = 'G:\小王子数据\derivation\FR\Masks\AllResampled_GreyMask_02_91x109x91.nii';
...
for stotyIndex = 1:9
    ...
    cd(subaddr)
    niiname = dir('*nii');
    fourDnii_head=spm_vol(niiname(1).name);
